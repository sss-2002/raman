...
class FileHandler:
    ...
    # 在 Streamlit 中创建文件上传逻辑
    def main():
        # 实例化 file_handler
        file_handler = FileHandler()

        zip_file = st.file_uploader("上传包含波数和光谱数据的压缩包", type=["zip"])

        if zip_file:
            try:
                wavenumbers, data = file_handler.load_data_from_zip(zip_file)
                st.success("数据加载成功")
                st.write("波数数据:", wavenumbers)
                st.write("光谱数据:", data)
            except Exception as e:
